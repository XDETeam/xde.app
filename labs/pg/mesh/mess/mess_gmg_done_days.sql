create view mess_gmg_done_days(month, week, date, day, hours, rolling, tasks) as
WITH cte AS (
    SELECT to_char(mess_gmg_done.at::timestamp with time zone, 'IYYY-MM'::text)      AS month,
    ...
    FROM mesh.mess_gmg_done
    GROUP BY mess_gmg_done.at
)
SELECT cte.month,
       cte.week,
       cte.date,
...
alter table mess_gmg_done_days
    owner to postgres;
